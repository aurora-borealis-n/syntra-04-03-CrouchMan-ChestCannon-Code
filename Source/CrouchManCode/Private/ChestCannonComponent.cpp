...
void UChestCannonComponent::Fire(FTransform origin, FVector direction)
{
	...
	if (cannonBallActor != nullptr)
	{
		...
		UCannonballComponent* cannonBallComponent = cannonBallActor->GetComponentByClass<UCannonballComponent>();
		if (cannonBallComponent != nullptr)
		{
			cannonBallComponent->Launch(direction * 50000);
		}
	} else{
		UE_LOG(LogTemp, Warning, TEXT("Can not fire in %s"), *GetNameSafe(this));
	}
